...
# SIMULATES DIPLOIDS
def sim_admix(nS=[10,10,10],nT=10,nO=10,tS=[0,0,0],tT=0,t2T=100,t23=200,t123=300,ta=[50,50],f=[.1,.1],tsO=[0.1,0.1,0.1,0.1], N = [10000]*8,mu=1.25e-8,r=1e-8,length=1000000,num_rep=1,coverage=10, err = None, seed=None, fs1s2 = None, fo1o2 = None, GToutput = False):
	#generate samples
	...
	#return whole simulations as an object or proceed to make GT output files
	if not GToutput:	
		return sims
	
	#initalize sample names
	sample_names = []
	pop = 0
	ind = 0
	cur_allele = 0
	for sample in samples:
		cur_pop = sample.population
		if cur_allele > 0:
			cur_allele = 0
			ind += 1
			continue
		if cur_pop != pop:
			pop = cur_pop
			ind = 0
		sample_names.append("pop_%i_sample_%i"%(cur_pop, ind))
		cur_allele += 1
	#set up error if necessary
	if err is None:
		err = np.zeros(len(sample_names))
	#initialize containers
	GT = [] #to hold the tree genotypes
	reads = [] #to hold the reads
	pos = []
	for chrom, sim in enumerate(sims):
		for variant in sim.variants():
			pos.append((chrom+1,round(variant.position)))
			var_array = variant.genotypes
			cur_GT = np.array([sum(var_array[i:(i+2)]) for i in range(0,len(samples)-1,2)])
			cur_reads = draw_reads(cur_GT,coverage,err)
			GT.append(cur_GT)
			reads.append(cur_reads)
	return sample_names, GT, reads, pos
...
def generate_plink_bed(names,outname,pos,GT=None,reads=None, r = 1e-8, duplicate = False):
	if GT is None and reads is None:
		...
		generate_plink_bed_GT(names,GT,pos,outname, r = r, duplicate = duplicate)
	elif reads is not None:
		generate_plink_bed_reads(names,reads,pos,outname, r = r, duplicate = duplicate)

def generate_plink_bed_GT(names,GT,pos,outname, r = 1e-8, duplicate = False):
	pos_included = set()
	...
	for i, site in enumerate(GT):
		...
		if cur_label not in pos_included:
			...
			continue
		cur_locus = pl.Locus(pos[i][0],cur_label,r*pos[i][1],pos[i][1],"A","T")
		outfile.write_row(cur_locus,site)
	outfile.close()

def generate_plink_bed_reads(names,reads,pos,outname, r = 1e-8, duplicate = False):
	pos_included = set()
	#create sample objects
...
def convert_plink_bed_to_packedancestrymap(infile,outfile=None,delete_par=True):
	if outfile is None: outfile = infile
	parfile = open("parfile_%s_%s.txt"%(infile,outfile),"w")
	parfile.write("genotypename: %s.bed\n"%infile)
	parfile.write("snpname: %s.bim\n"%infile)
	parfile.write("indivname: %s.fam\n"%infile)
	parfile.write("outputformat: PACKEDANCESTRYMAP\n")
	parfile.write("genotypeoutname: %s.packedancestrymapgeno\n"%outfile)
	parfile.write("snpoutname: %s.snp\n"%outfile)
	parfile.write("indivoutname: %s.ind\n"%outfile)
	parfile.write("familynames: NO")
	parfile.flush()
	os.system("convertf -p parfile_%s_%s.txt"%(infile,outfile))
	if delete_par:
		os.remove("parfile_%s_%s.txt"%(infile,outfile))
...
def generate_beagle_GL(names,reads,err,pos,outname,duplicate=False, precision=3):
	pos_included = set()
