...
parser.add_argument('--writeGT', help="Write raw GT/GL", required=False, default=True, action="store_true")
...
writeGT = args.writeGT
outpop_inds=sample_size
...
tree_seq_iter = sim_pop_ngs.sim_admix(t2T=divergence/5,t23=divergence/2,t123=divergence,ta=[50,50],f=[p1,p2],num_rep=nchr,nS=[sample_size_internal]*3,nT=sample_size_internal,nO=outpop_inds_internal,seed=seed,mu=2.5e-8,r=2e-8,length=num_bases)
...
if 1:
	...
	for x in popindexes:
		ind=ind+1
		sample_names.append("pop_%s_sample_%s"%(x, ind))
...
for chrom, sim in enumerate(tree_seq_iter):
	...
	if 1:
		anc_seq = sim_pop_ngs.randomDNA(num_bases)
		chr_sequences = [anc_seq] * ancient_individuals * 2
		for i in range(len(chr_sequences)):
			chr_sequences[i]=list(chr_sequences[i])
		for variant in sim.variants():
			pos.append((chrom+1,round(variant.position)))
			var_array = variant.genotypes
			cur_GT = np.array([sum(var_array[i:(i+2)]) for i in range(0,len(sample_names)*2-1,2)])
			cur_reads = sim_pop_ngs.draw_reads(cur_GT,1000,err)
			GT.append(cur_GT)
			reads.append(cur_reads)
			alleles.append((anc_seq[int(pos[-1][1])],sim_pop_ngs.mutate(anc_seq[int(pos[-1][1])])))		
			for j in range(ancient_individuals * 2):
				chr_sequences[j][int(pos[-1][1])]=alleles[-1][var_array[j]]
	...
	for j in range(ancient_individuals):
		...
		for i in range(2):
			...
			with open(anc_dir + filename, 'w') as f:
				...
				f.write('>chr%s\n' %(chrom + 1))
				# Write sequence
				f.write(''.join(chr_sequences[chr_index])+'\n')

			chr_index += 1

if writeGT:

	sim_pop_ngs.generate_plink_bed(sample_names,"GT",pos,alleles,GT=GT,r=1e-8)
	sim_pop_ngs.generate_beagle_GL(sample_names, reads, 0.0, pos,alleles, "GL")
